...
# Song info variables
ALBUM_TEXT_KEYS = ["Artist","Song","Album","Release Date","Duration", "Lyrics"]
    ...
    try:
        ...
        while True:
            ...
            if correlation < SIM_THRESHOLD:
                ...
                api_response = send.send_auddio_api_request(api_token=API_TOKEN, audio_data=mp3_encoding)
                ...
                # Get the sample lyrics, if available.
                song_lyrics = send.send_lyric_api_request(audio_uri=api_response_parsed.get("preview_url"))
                # Truncate song lyrics
                song_lyrics = send.truncate_audio_transcript(audio_transcript=song_lyrics)
                
                # Add song lyrics to api response
                api_response_parsed["Lyrics"] = song_lyrics
                ...
                if api_response_parsed is not None:
